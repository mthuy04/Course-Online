...
import { 
  ...
  TrendingUp, Zap, ShieldCheck, Gift, Target, BrainCircuit 
} from 'lucide-react';
...
const CourseCard3D = ({ course, onAction, onDetail }) => (
  <div onClick={() => onDetail(course)} className="group relative bg-white rounded-[24px] border border-slate-100 overflow-hidden hover:shadow-[0_20px_50px_-12px_rgba(79,70,229,0.2)] transition-all duration-500 hover:-translate-y-2 cursor-pointer h-full flex flex-col">
    <div className="relative h-48 overflow-hidden flex-shrink-0">
      ...
      <span className="absolute top-3 left-3 bg-white/90 backdrop-blur-md px-3 py-1 rounded-full text-[10px] font-black uppercase tracking-wider text-slate-900 shadow-lg">{course.level === 'cap1' ? 'Tiểu học' : course.level === 'cap2' ? 'THCS' : 'THPT'}</span>
      <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300 bg-black/10 backdrop-blur-[1px]"><button className="bg-white text-slate-900 px-4 py-2 rounded-full font-bold flex items-center gap-2 shadow-xl transform scale-90 group-hover:scale-100 transition-transform"><Eye size={18}/> Xem ngay</button></div>
    </div>
    <div className="p-5 flex flex-col flex-1">
      <div className="flex justify-between items-center mb-3"><span className="text-[10px] font-bold text-indigo-500 bg-indigo-50 px-2 py-1 rounded-lg flex items-center gap-1"><Clock size={12}/> {course.duration || '20h'}</span><span className="text-amber-500 text-xs font-black flex items-center gap-1">★ 4.9</span></div>
      ...
      <div className="mt-auto flex items-center justify-between pt-4 border-t border-slate-50"><p className="text-lg font-black text-indigo-600">{formatMoney(course.price)}</p><button onClick={(e) => { e.stopPropagation(); onAction(course); }} className="p-2.5 rounded-xl bg-slate-900 text-white shadow-lg hover:bg-indigo-600 transition-all hover:scale-105 active:scale-95"><Plus size={18}/></button></div>
    </div>
  </div>
);

const HomeDashboard = () => {
  const data = [{name:'T2',h:2},{name:'T3',h:4},{name:'T4',h:1},{name:'T5',h:5},{name:'T6',h:3},{name:'T7',h:6},{name:'CN',h:2}];
  return (
    <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 mb-10">
      <div className="lg:col-span-2 bg-white p-6 rounded-[32px] border border-slate-100 shadow-sm flex flex-col justify-center">
        <div className="flex justify-between items-center mb-6">
            <h3 className="font-bold text-slate-800 flex items-center gap-2"><TrendingUp size={20} className="text-indigo-600"/> Hoạt động tuần qua</h3>
            <span className="text-xs font-bold text-green-600 bg-green-50 px-2 py-1 rounded-lg">+12% so với tuần trước</span>
        </div>
        <div className="h-56"><ResponsiveContainer width="100%" height="100%"><BarChart data={data}><XAxis dataKey="name" axisLine={false} tickLine={false}/><Tooltip cursor={{fill:'transparent'}} contentStyle={{borderRadius:'12px', border:'none', boxShadow:'0 4px 20px rgba(0,0,0,0.1)'}}/><Bar dataKey="h" radius={[10,10,10,10]} barSize={30}><Cell fill="#e2e8f0"/><Cell fill="#e2e8f0"/><Cell fill="#e2e8f0"/><Cell fill="#e2e8f0"/><Cell fill="#4f46e5"/><Cell fill="#e2e8f0"/><Cell fill="#e2e8f0"/></Bar></BarChart></ResponsiveContainer></div>
      </div>
...
// --- MAIN VIEW ---
const StudentView = ({ page, setPage, courses, cart, myCourses, handleAddToCart, handlePayment, removeFromCart, onOpenDetail, onOpenLearning }) => {
  ...
  // 3. MY LEARNING
  if (page === 'my-learning') return (
    <div className="space-y-10 animate-fade-in-up pb-10">
        <h2 className="text-3xl font-black text-slate-800">Góc học tập</h2>
        <div className="grid grid-cols-2 md:grid-cols-4 gap-6">
            <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm"><p className="text-xs text-slate-500 font-bold uppercase mb-1">Khóa đang học</p><h3 className="text-3xl font-black text-indigo-600">{myCourses.length}</h3></div>
            <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm"><p className="text-xs text-slate-500 font-bold uppercase mb-1">Giờ đã học</p><h3 className="text-3xl font-black text-emerald-600">12.5h</h3></div>
            <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm"><p className="text-xs text-slate-500 font-bold uppercase mb-1">Bài tập đã làm</p><h3 className="text-3xl font-black text-orange-500">24</h3></div>
            <div className="bg-white p-5 rounded-[24px] border border-slate-100 shadow-sm"><p className="text-xs text-slate-500 font-bold uppercase mb-1">Điểm trung bình</p><h3 className="text-3xl font-black text-blue-600">8.8</h3></div>
        </div>

        {myCourses.length === 0 ? <div className="text-center py-24 bg-white rounded-[32px] border-2 border-dashed border-slate-200"><p className="text-slate-400 font-bold text-lg">Bạn chưa đăng ký khóa học nào.</p><button onClick={()=>setPage('home')} className="mt-4 text-indigo-600 font-bold hover:underline">Tìm khóa học ngay</button></div> : 
          <div className="grid grid-cols-1 md:grid-cols-3 gap-8">
            {myCourses.map(c => (
    <div key={c.id} className="bg-white p-6 rounded-[32px] border border-slate-100 shadow-xl group">
        <div className="relative overflow-hidden rounded-2xl mb-6">
            ...
            <div className="absolute inset-0 bg-black/20 group-hover:bg-black/40 transition-colors flex items-center justify-center opacity-0 group-hover:opacity-100">
                <button 
                    onClick={() => onOpenLearning(c)} // Gọi hàm mở màn hình học
        ...
        <h3 className="text-xl font-black text-slate-900 mb-2 line-clamp-1">{c.title}</h3>
        
        {/* Nút bấm chính phía dưới */}
        <button 
            onClick={() => onOpenLearning(c)} // Đảm bảo gọi onOpenLearning
            className="w-full py-3.5 bg-slate-900 text-white rounded-xl font-bold hover:bg-indigo-600 transition-colors"
        >
            Tiếp tục học
        </button>
                </div>
  ...
  // 4. CART
  if (page === 'cart') return (
    <div className="max-w-7xl mx-auto animate-fade-in-up py-10">
        <h2 className="text-3xl font-black text-slate-800 mb-8">Giỏ hàng của bạn ({cart.length})</h2>
        <div className="flex flex-col lg:flex-row gap-8">
            <div className="w-full lg:w-3/4 space-y-6">
                {cart.length === 0 && <p className="text-slate-500 text-lg">Giỏ hàng đang trống.</p>}
                {cart.map((c, i) => (
                    <div key={i} className="flex gap-6 p-5 bg-white rounded-3xl border border-slate-100 shadow-sm items-center hover:shadow-md transition-all">
                        <img src={c.image} className="w-32 h-24 object-cover rounded-2xl shadow-sm"/>
                        <div className="flex-1"><h4 className="text-xl font-black text-slate-900 mb-1">{c.title}</h4><p className="text-indigo-600 font-bold text-xl">{formatMoney(c.price)}</p></div>
                        <button onClick={() => removeFromCart(i)} className="p-3 bg-rose-50 text-rose-500 rounded-2xl hover:bg-rose-500 hover:text-white transition-all"><Trash2 size={20}/></button>
                    </div>
                ))}
            </div>
            {cart.length > 0 && (
              <div className="w-full lg:w-1/4 h-fit">
                  <div className="bg-white p-8 rounded-[32px] shadow-2xl shadow-slate-200 border border-slate-100 sticky top-6">
                      <h3 className="text-2xl font-black text-slate-900 mb-6">Tóm tắt</h3>
                      <div className="space-y-4 mb-8">
                        <div className="flex justify-between text-slate-500 font-medium"><span>Tạm tính</span><span>{formatMoney(cart.reduce((t,c)=>t+parseInt(c.price),0))}</span></div>
                        <div className="flex justify-between text-slate-500 font-medium"><span>Giảm giá</span><span>0 đ</span></div>
                        <div className="border-t border-slate-100 pt-4 flex justify-between items-center"><span className="text-slate-800 font-bold">Tổng cộng</span><span className="text-2xl font-black text-indigo-600">{formatMoney(cart.reduce((t,c)=>t+parseInt(c.price),0))}</span></div>
                      </div>
                      <button onClick={handlePayment} className="w-full py-4 bg-gradient-to-r from-indigo-600 to-violet-600 text-white rounded-2xl font-black text-lg shadow-xl shadow-indigo-200 hover:scale-105 transition-all">Thanh toán ngay</button>
